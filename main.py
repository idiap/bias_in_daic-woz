...
DATASETS = [("data/AVEC_16_data/train_Ellie.txt", "data/AVEC_16_data/dev_Ellie.txt"),
            ("data/AVEC_16_data/train_Participant.txt", "data/AVEC_16_data/dev_Participant.txt")]
FEATURES = [-1, 250, 0]
OPTIONS = list(itertools.product(DATASETS, FEATURES, (True, False)))
OPTIONS_N = len(OPTIONS)


def get_task_info(option, job_ix=None, only_description=False):
    dataset, feature_size, use_pagerank = option
    if feature_size == 0:
        feature_selection = False
    else:
        feature_selection = True
    study_name = f"induct-gcn[{'pagerank' if use_pagerank else 'original'}-features{feature_size}]"
    if job_ix is not None:
        study_name = f"{job_ix}_" + study_name
...
def train(model,
          ...
          output_path = None,
          final=False):
    output_path = output_path or PATH_OUTPUT
    model.to(DEVICE)
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % valid_period == 0:
            ...
            if final:
                save_metrics(os.path.join(output_path, 'induct-gcn-metric-checkpoint.pkl'), train_loss_list, steps_list, target_metric_list)
            ...
            if target_metric > best_target_metric:
                ...
                no_improvement_counter = 0
                save_checkpoint(os.path.join(output_path, 'induct-gcn-model-checkpoint.pkl'), model, target_metric)
...
def train_inductgcn(learning_rate, num_steps, save_model=False):
    ...
    if FEATURE_SELECTION:
        ...
        try:
            vocab = doc_vectorizer.get_feature_names()
        except AttributeError:
        ...
        if FEATURE_SIZE == -1:
            ...
            X_train_feat = doc_vectorizer.transform(X_train)
            k = FEATURE_SIZE if FEATURE_SIZE <= X_train_feat.shape[1] else "all"
            selector = SelectKBest(f_classif, k=k).fit(X_train_feat, y_train_list)
    ...
    try:
        best_value, best_results = train(model, y_train, X_dev, y_dev, optimizer, class_weight,
                                         output_path=PATH_OUTPUT,
                                         num_epochs=num_steps, valid_period=VALIDATION_PERIOD,
                                         final=save_model)
...
if __name__ == "__main__":
    ...
    DATASET, FEATURE_SIZE, USE_PAGERANK, FEATURE_SELECTION, STUDY_NAME, description = get_task_info(OPTIONS[task_ix], task_ix)
    ...
    PATH_OUTPUT = f"output/{dataset_type}/{STUDY_NAME}/"
    PATH_OUTPUT_MODEL =f"model/{dataset_type}/{STUDY_NAME}/"
    ...
    best_results = None
    best_model = None
    best_doc_vectorizer = None

    def objective(trial):
        global best_metric, best_results, best_model, best_doc_vectorizer
        ...
        current_metric, current_best_results, model, doc_vectorizer = train_inductgcn(lr, num_steps, False)
        ...
        (TARGET_METRIC != "loss" and current_metric > best_metric):
            ...
            load_checkpoint(os.path.join(PATH_OUTPUT, 'induct-gcn-model-checkpoint.pkl'), model)
            best_model = model
            best_doc_vectorizer = doc_vectorizer

            print("Saving new global best model to disk...")
            best_model.save(X_dev, os.path.join(PATH_OUTPUT_MODEL, f'model_inductgcn[{FEATURE_SIZE}].pkl'))
            with open(os.path.join(PATH_OUTPUT_MODEL, f'vtzer_inductgcn[{FEATURE_SIZE}].pkl'), 'wb') as fout:
                pickle.dump(best_doc_vectorizer, fout)
    ...
    with open(os.path.join(PATH_OUTPUT, "results.json"), "w") as writer:
        json.dump(best_results, writer)

    with open(os.path.join(PATH_OUTPUT, f"words.csv"), "w") as writer:
        H_1 = best_model.get_H_1(best_model.Conv_0)
        z_logits = best_model.node_emb2out(torch.mm(best_model.A_norm, H_1))
        ix2word = {}
        for word in best_doc_vectorizer.vocabulary_:
            ix2word[best_doc_vectorizer.vocabulary_[word]] = word
        for ix in ix2word:
            label_ix = z_logits[ix].argmax().item()
            label = ix2label[label_ix]
            writer.write(f"{ix2word[ix]},{label},{nn.Softmax()(z_logits[ix])[label_ix]}\n")

    evaluate(best_model, X_dev, y_dev, labels, show_result=True)
    best_model.eval()
    with torch.no_grad():
        output = best_model(X_dev)
        y_pred = torch.argmax(output, axis=-1).tolist()
    with open(os.path.join(PATH_OUTPUT, "results_predictions.json"), "w") as writer:
        json.dump({"y_pred": y_pred, "y_true": y_dev.tolist()}, writer)

    if PATH_TEST:
        print("==================   Evaluation on TEST SET  ============================")
        print("Test set")
        best_model.Conv_0_Test = None
        evaluate(best_model, X_test, y_test, labels, show_result=True)
        test_results = evaluate(best_model, X_test, y_test, labels, show_result=False)
        with open(os.path.join(PATH_OUTPUT, "test_results.json"), "w") as writer:
            json.dump(test_results, writer)
